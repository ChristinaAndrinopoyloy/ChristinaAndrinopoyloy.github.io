// Translation data for bilingual support
const translations = {
    el: {
        // Hero Section
        "name": "Χριστίνα Ανδρινοπούλου",
        "title": "Full-Stack & AI Engineer",
        "value-proposition": "Σχεδιασμός software συστημάτων που αξιοποιούν ουσιαστικά δεδομένα και AI",
        
        // Navigation
        "nav-about": "Σχετικά",
        "nav-experience": "Εμπειρία",
        "nav-education": "Εκπαίδευση",
        "nav-research": "Έρευνα",
        "nav-skills": "Δεξιότητες",
        
        // About Section
        "about-title": "Σχετικά με εμένα",
        "about-text": "Full-Stack & AI Engineer με ενασχόληση στην ανάπτυξη λογισμικού, τη μηχανική μάθηση, την επιστήμη και ανάλυση δεδομένων. Σχεδιασμός και υλοποίηση ολοκληρωμένων software συστημάτων, με συνδυασμό σύγχρονων τεχνικών AI και δοκιμασμένων πρακτικών ανάπτυξης λογισμικού, με στόχο αξιόπιστες λύσεις που λειτουργούν στην πράξη.\n\nΤα σύνθετα προβλήματα προσεγγίζονται ολιστικά - από τα δεδομένα και τα μοντέλα έως την αρχιτεκτονική, την υλοποίηση και την ανάπτυξη σε πραγματικά περιβάλλοντα.",
        
        // Experience
        "experience-title": "Εμπειρία",
        "experience-period": "2020 - Σήμερα · 4 χρόνια",
        "job-title": "Full Stack Software Developer",
        "job-description": "Σχεδιασμός και ανάπτυξη full-stack εφαρμογών, με έμφαση στην επίλυση σύνθετων προβλημάτων και την παράδοση καινοτόμων λύσεων που βελτιώνουν την αποδοτικότητα και την εμπειρία χρήστη.",
        
        // Education
        "education-title": "Εκπαίδευση",
        "msc-degree": "Μεταπτυχιακό στην Επιστήμη Δεδομένων και Τεχνολογίες Πληροφοριας",
        "msc-field": "Ειδίκευση: Βιοπληροφορική και Βιοϊατρική Επιστήμη Δεδομένων",
        "university": "Εθνικό και Καποδιστριακό Πανεπιστήμιο Αθηνών",
        "bsc-degree": "Πτυχίο Πληροφορικής και Τηλεπικοινωνιών",
        
        // Research
        "research-title": "Ερευνητικές Δημοσιεύσεις",
        "read-paper": "Διάβασε το Paper",
        
        // Interests
        "interests-title": "Πεδία Ενδιαφέροντος",
        "skill-ai": "Τεχνητή Νοημοσύνη",
        "skill-ml": "Μηχανική Μάθηση",
        "skill-ds": "Επιστήμη Δεδομένων",
        "skill-da": "Ανάλυση Δεδομένων",
        "skill-dev": "Ανάπτυξη Λογισμικού",
        "skill-bio": "Βιοπληροφορική",
        
        // Footer
        "footer-text": "© 2026 Χριστίνα Ανδρινοπούλου. Όλα τα δικαιώματα κατοχυρωμένα."
    },
    
    en: {
        // Hero Section
        "name": "Christina Andrinopoulou",
        "title": "Full-Stack & AI Engineer",
        "value-proposition": "Designing software systems where data and AI converge",
        
        // Navigation
        "nav-about": "About",
        "nav-experience": "Experience",
        "nav-education": "Education",
        "nav-research": "Research",
        "nav-skills": "Skills",
        
        // About Section
        "about-title": "About Me",
        "about-text": "A Full-Stack & AI Engineer working across software development, machine learning, data science, and data analysis. Focused on designing and delivering end-to-end software systems, combining applied intelligence with solid engineering practices to build reliable, production-ready solutions.\n\nComplex problems are approached holistically - from data and models to architecture, implementation, and deployment.",
        
        // Experience
        "experience-title": "Experience",
        "experience-period": "2020 - Present · 4 years",
        "job-title": "Full Stack Software Developer",
        "job-description": "Leading role in developing full-stack applications, with emphasis on solving complex problems and delivering innovative solutions that improve efficiency and user experience.",
        
        // Education
        "education-title": "Education",
        "msc-degree": "MSc in Data Science and Information Technologies",
        "msc-field": "Specialization: Bioinformatics & Biomedical Data Science",
        "university": "National and Kapodistrian University of Athens",
        "bsc-degree": "BSc in Computer Science",
        
        // Research
        "research-title": "Research Publications",
        "read-paper": "Read Paper",
        
        // Interests
        "interests-title": "Areas of Interest",
        "skill-ai": "Artificial Intelligence",
        "skill-ml": "Machine Learning",
        "skill-ds": "Data Science",
        "skill-da": "Data Analysis",
        "skill-dev": "Software Development",
        "skill-bio": "Bioinformatics",
        
        // Footer
        "footer-text": "© 2026 Christina Andrinopoulou. All rights reserved."
    }
};
